...
export default class ChatAreaWrite extends Component {
    ...
    handleSendMsg = (e) => {
        if(e.keyCode === 13 && window.document.getElementById('area-input-text').value !== '') {
            ...
            this.handleButtonSendAndAudio();
            
            const areaInputText = window.document.getElementById('area-input-text');
            
            areaInputText.removeEventListener('keydown', this.handleSendMsg);

            setTimeout(() => {
                areaInputText.addEventListener('keydown', this.handleSendMsg);
            }, 1000);
        }
    }
    ...
    render() {
        return (
            <>
                ...
                <input type="text" placeholder="Digite uma mensagem" onChange={this.handleButtonSendAndAudio} id="area-input-text" />
...
export const chatAreaWriteObj = new ChatAreaWrite();
